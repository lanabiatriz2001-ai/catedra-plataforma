...
struct SimuladoView: View {
    ...
    // CRONÔMETRO como publisher (mesmo molde de OralBancasView): o Timer.scheduledTimer
    // guardado em @State vazava — capturava `prova` por valor, seguia escrevendo num
    // @State órfão se a view fosse recriada e não parava ao abrir uma prova encerrada.
    // Com onReceive só há tick enquanto a view está montada e a prova está aberta.
    private let relogio = Timer.publish(every: 1, on: .main, in: .common).autoconnect()

    private var disciplinas: [String] { store.disciplinasEm(store.entries).map(\.nome) }

    var body: some View {
        SectionShell(icon: Selecao.simulado.simbolo, title: Selecao.simulado.titulo,
                     subtitle: "Itens sorteados do acervo oficial (parte com a tese invertida, para julgar como ERRADO) e discursivas de provas reais com o espelho da banca. Tudo roda no aparelho.",
                     trailing: AnyView(HStack(spacing: 6) {
                         EtiquetaEstudo(texto: "Certo/Errado + discursivas")
                         EtiquetaEstudo(texto: "Local · sem IA", cor: Palette.importante)
                     })) {
            ScrollView {
    ...
    private var configuracao: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Flow(espacamento: 6) {
                    JurisChip(texto: "Todas", ativo: disciplina == nil) { disciplina = nil }
                    JurisChipsLimitados(itens: disciplinas, limite: 18, rotulo: { $0 }, ativo: { disciplina == $0 }) { disciplina = $0 }
                }
            }
            Button {
    ...
    private func stepper(_ rotulo: String, _ v: Binding<Int>, _ faixa: ClosedRange<Int>, passo: Int) -> some View {
        HStack(spacing: 8) {
            ...
            Text("\(v.wrappedValue)").font(Typo.num(18, .heavy)).foregroundStyle(Palette.titleInk).frame(minWidth: 30)
                ...
                ForEach(historico.prefix(12)) { h in
                    HStack(spacing: 12) {
                        ...
                        Text("\(h.acertos)/\(h.itens.count)").font(Typo.num(15, .heavy))
                            .foregroundStyle(h.percentual >= 70 ? Palette.ok : (h.percentual >= 50 ? Palette.warn : Palette.secondaryInk))
                        Button("Abrir") { prova = h; visao = h.encerrado ? .relatorio : .enunciados }
        ...
        VStack(alignment: .leading, spacing: 10) {
            Flow(espacamento: 8) {
                ForEach(SimuladoVisao.allCases) { v in
                    JurisChip(texto: v.rawValue, simbolo: v.simbolo, ativo: visao == v) { visao = v }
                }
            }
            Flow(espacamento: 10) {
                ...
                Button { exportar(p) } label: { Label("Exportar / imprimir", systemImage: "square.and.arrow.up") }
                    .buttonStyle(.bordered).tint(Palette.accent)
                Button("Voltar") { salvarProva(); prova = nil; historico = SimuladoCache.todos() }
    ...
    private func relatorio(_ p: SimuladoProva) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            Flow(espacamento: 22) {
                JurisKPI(valor: String(format: "%.0f%%", p.percentual), rotulo: "acerto", cor: p.percentual >= 70 ? Palette.ok : (p.percentual >= 50 ? Palette.warn : Palette.bad))
                JurisKPI(valor: "\(p.acertos)", rotulo: "acertos", cor: Palette.ok)
                JurisKPI(valor: "\(p.erros)", rotulo: "erros", cor: Palette.bad)
                JurisKPI(valor: "\(p.emBranco)", rotulo: "em branco")
                JurisKPI(valor: SimuladoLocal.tempo(p.segundos), rotulo: "tempo")
            }
            if porDisc.count > 1 {
    ...
    private func itemCard(_ n: Int, _ it: SimuladoItem, _ p: SimuladoProva) -> some View {
        ...
        let cor = mostraGab ? (it.gabarito ? Palette.ok : Palette.bad) : Palette.accent
            ...
            HStack(spacing: 8) {
                ...
                if mostraGab {
                    ...
                    if visao == .relatorio, let r = resp {
                        EtiquetaEstudo(texto: r == it.gabarito ? "Você acertou" : "Você errou", cor: r == it.gabarito ? Palette.ok : Palette.bad)
    ...
    private func botaoCE(_ t: String, _ valor: Bool, _ resp: Bool?, _ it: SimuladoItem, _ p: SimuladoProva) -> some View {
        ...
        return Button {
            ...
            Text(t).font(.system(size: 12.5, weight: .bold))
                ...
                .foregroundStyle(ativo ? Color.white : Palette.titleInk)
        }
        .buttonStyle(.plain)
    ...
    private func salvarProva() {
        if let p = prova { SimuladoCache.salvar(p) }
    }
}
