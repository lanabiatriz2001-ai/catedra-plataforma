                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            ...
                            Button("Ver todas") { openUpdates() }
                                .buttonStyle(.borderless)   // .link não existe no iPadOS
                        }
...
struct FlashcardsManagerSheet: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if cards.isEmpty {
                // ContentUnavailableView só existe do iOS 17 para cima e o app suporta o 16.
                VStack(spacing: 8) {
...
struct AnkiExportSheet: View {
    ...
    private func save() {
        ...
        // No iPadOS não existe "escolher a pasta" como no Mac. Grava na pasta Documentos
        // do próprio app, que aparece no app Arquivos porque o Info.plist declara
        // UIFileSharingEnabled — de lá dá para mandar para o Anki, iCloud ou e-mail.
        ...
        if ok > 0 {
            // Sem equivalente a "revelar no Finder" no iPadOS — a mensagem diz onde ficou.
            message = ok == files.count
                ? "\(ok) arquivo(s) salvos em Arquivos ▸ No meu iPad ▸ Cátedra. Importe cada um no Anki."
                : "\(ok) de \(files.count) salvos em Arquivos ▸ Cátedra. Falha em \(files.count - ok): \(lastErr ?? "erro desconhecido")."
        } else {
            message = "Não foi possível salvar: \(lastErr ?? "erro desconhecido")."
        }
    }
}
