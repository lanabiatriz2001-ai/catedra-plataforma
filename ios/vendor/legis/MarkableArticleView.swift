...
/// Renderiza UM artigo (fatia do texto completo da lei) num UITextView que permite
...
struct MarkableArticleView: UIViewRepresentable {
    let fullText: String
    ...
    // Retorna a UITextView DIRETO (sem NSScrollView interno). O artigo é renderizado
    ...
    func makeUIView(context: Context) -> ReaderTextView {
        ...
        // UITextView não tem isRichText, isVerticallyResizable nem drawsBackground. No
        // iPadOS o inset é UIEdgeInsets, o fundo se apaga pela cor, e a rolagem PRÓPRIA
        // fica desligada — quem rola é o ScrollView do SwiftUI que envolve o Estudo
        // (mesma intenção do original, que devolvia a text view sem NSScrollView).
        textView.textContainerInset = UIEdgeInsets(top: 10, left: 34, bottom: 10, right: 34)
    ...
    private func remeasure(_ textView: ReaderTextView, coordinator: Coordinator, force: Bool) {
        ...
        let inset = textView.textContainerInset   // UIEdgeInsets: tem lados, não largura/altura
        container.widthTracksTextView = false
        container.size = CGSize(width: max(1, width - inset.left - inset.right),
                                height: CGFloat.greatestFiniteMagnitude)
        lm.ensureLayout(for: container)
        let height = ceil(lm.usedRect(for: container).height + inset.top + inset.bottom)
    ...
    private func updateCommentAnchors(lm: NSLayoutManager, container: NSTextContainer, inset: UIEdgeInsets) {
        ...
        for a in annotations where !a.isOrphaned && !a.note.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            out.append(ArticleCommentAnchor(id: a.id, y: rect.minY + inset.top, note: a.note, colorHex: a.colorHex))
        }
        out.sort { $0.y < $1.y }
    ...
    private func baseFont(bold: Bool) -> NSFont {
        ...
        case "Sistema (Serifa)":
            if let d = NSFont.systemFont(ofSize: size, weight: bold ? .semibold : .regular).fontDescriptor.withDesign(.serif) {
                return UIFont(descriptor: d, size: size)   // init nao-opcional no iPadOS
            }
            return bold ? .boldSystemFont(ofSize: size) : .systemFont(ofSize: size)
        default:
            // Não há NSFontManager no iPadOS: a família vira um descriptor e o negrito
            // entra como trait simbólico.
    ...
    /// Versão itálica da fonte (o macOS fazia por NSFontManager.convert).
    private func italico(_ f: NSFont) -> NSFont {
        guard let d = f.fontDescriptor.withSymbolicTraits(.traitItalic) else { return f }
    ...
    private func applyText(to textView: ReaderTextView, coordinator: Coordinator) {
        ...
        // Mesma chave calculada em updateUIView; inclui o accent para repintar os
    ...
    private func colorizeStructure(_ storage: NSTextStorage) {
        ...
        while i < ns.length {
            ...
            if line.range(of: "^(§+\\s*\\d+[ºo°]?|Parágrafo)", options: .regularExpression) != nil {
                ...
                colorPrefix("^[a-z]\\)", .secondaryLabel, bold: false)
    ...
    private func applyAnnotations(to textView: ReaderTextView, coordinator: Coordinator) {
        let storage = textView.textStorage   // nao-opcional no iPadOS
        ...
        for a in localAnnotations() {
            ...
            case .italic:
                storage.addAttribute(.font, value: italico(baseF), range: range)
            case .textColor:
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        ...
        func textViewDidChangeSelection(_ notification: Notification) {
            ...
            if len > 0 {
                let lm = tv.layoutManager, container = tv.textContainer
                ...
                rect = CGRect(x: br.minX + inset.left, y: br.minY + inset.top,
