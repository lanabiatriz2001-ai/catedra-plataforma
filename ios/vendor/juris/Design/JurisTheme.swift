...
extension Color {
    ...
    var hexString: String {
        ...
        guard NSColor(self).getRed(&cr, green: &cg, blue: &cb, alpha: &ca) else { return "#000000" }
        return String(format: "#%02X%02X%02X",
                      Int((cr * 255).rounded()), Int((cg * 255).rounded()), Int((cb * 255).rounded()))
    }

    /// Cor dinâmica que se adapta a claro/escuro.
    static func dynamic(light: String, dark: String) -> Color {
        // No macOS a cor dinâmica consultava a `appearance`; no iPadOS vem do trait.
