...
struct EntryDetailView: View {
    ...
    /// lockFocus; no iPadOS o desenho fora da tela é UIGraphicsImageRenderer — e a imagem
    /// precisa de .alwaysOriginal, senão o sistema a pinta de template e some a cor.
    private func corSwatch(_ hex: String, size: CGFloat = 11) -> Image {
        let img = UIGraphicsImageRenderer(size: CGSize(width: size, height: size)).image { _ in
            ...
            NSColor.separator.setStroke(); path.lineWidth = 0.5; path.stroke()
    ...
    private func inserirNoVerbete(_ s: String) {
        ...
        // shouldChangeText/didChangeText são do NSTextView. No iPadOS edita-se o storage
        // direto e avisa-se o delegate na mão — sem o aviso, o rascunho não seria salvo.
